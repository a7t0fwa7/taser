...
from taser.http import get_request, get_statuscode, get_title, extract_header
...
def do_http_req(target, protocol, port, timeout):
    ...
    resp = get_request(url, timeout=timeout, screenshot=args.screenshot, proxies=args.proxy,  browser=args.browser, install=args.auto_install)
...
if __name__ == '__main__':
    ...
    browser = args.add_argument_group("Browser Simulation")
    browser.add_argument('--browser', action='store_true', help="Use Selenium Web Browser")
    browser.add_argument('--auto-install', action='store_true', help="Install chrome driver")
    browser.add_argument('--screenshot', type=str, default=False, help='Screenshot directory (False)*')
